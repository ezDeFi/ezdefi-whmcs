jQuery(function($) {
  ...
  var whmcs_ezdefi_qrcode = function() {
    ...
    this.paymentData = JSON.parse(
      this.$container.find(selectors.paymentData).text()
    );
  ...
  whmcs_ezdefi_qrcode.prototype.onActivateTab = function(method, panel) {
    ...
    self.createEzpayPayment
      ...
      .error(function() {
        panel.text(
          "Something wrong happens. Please contact support for more information."
        );
        self.$loader.hide();
        self.$tabs.show();
        self.$currencySelect.show();
  ...
  whmcs_ezdefi_qrcode.prototype.success = function() {
    ...
    setTimeout(function() {
      window.location = self.urlData.clientArea;
    }, 3000);
  };
